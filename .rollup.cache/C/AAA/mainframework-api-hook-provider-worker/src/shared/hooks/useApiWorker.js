...
const responseQueue = {};
const updater = (n) => n + 1;
const ERROR_KINDS = ["http", "network", "validation"];
const isWorkerErrorPayload = (d) => !!d &&
    typeof d === "object" &&
    "kind" in d &&
    typeof d.kind === "string" &&
    ERROR_KINDS.includes(d.kind) &&
    "message" in d &&
...
apiWorker.onmessage = (event) => {
    ...
    if (!entry)
        return;
    if (cacheName !== "error" && normalizeKey(entry.cacheName) !== normalizeKey(cacheName))
        return;
    if (errorPayload) {
        ...
        entry.error = isWorkerErrorPayload(errorPayload)
            ...
            : {
                kind: "validation",
                message,
                ...(event.data.data?.code !== undefined && {
